    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {!isLoading && !isError && rows.length === 0 && (
        <EmptyState
          ...
          hint="No upcoming earnings in the researched universe over the selected horizon. Try a longer window or refresh."
        />
      )}

      {chart && (
        <div>
          <SectionTitle>Ranked candidates (confidence)</SectionTitle>
          <Card>
      ...
      {rows.length > 0 && (
        <div>
          <SectionTitle>Trade detail</SectionTitle>
          <Card>
          ...
          <p className="mt-2 flex items-start gap-1.5 text-[0.7rem] text-muted-foreground">
...
function Row({ r }: { r: ScanCandidate }) {
  ...
  return (
    <tr className="border-b border-border/50 hover:bg-accent/30">
      <td className="px-3 py-2.5 font-semibold">{r.ticker}</td>
